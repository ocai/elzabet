...
describe('User: create', () => {
    ...
    test('returns a user with default points of 100', async() => {
        const res = await user.create({'username': 'snorlax_tspins', 'discordId': '40127402917470'});
        expect(res).toMatchObject({
            'id': expect.any(Number),
            'username': 'snorlax_tspins',
            'discordId': '40127402917470',
...
describe('User: updateUser', () => {
    test('returns a user correctly when adding points', async () => {
        const [userId] = await dbConn('users').insert(f.user({'username': 'snorlax', 'points': 0}));
    ...
    test('returns a user correctly when subtracting points', async () => {
        const [userId] = await dbConn('users').insert(f.user({'username': 'snorlax', 'points': 1000}));
    ...
    test('actually updates the user in the DB', async () => {
        const [userId] = await dbConn('users').insert(f.user({'username': 'snorlax', 'points': 7000}));
...
describe('User: updateAll', () => {
    test('updates every user', async () => {
        ...
        const [userId1] = await dbConn('users').insert(f.user({'username': 'snorlax', 'points': 100}));
        const [userId2] = await dbConn('users').insert(f.user({'username': 'L0Liver', 'points': 400}));
        const [userId3] = await dbConn('users').insert(f.user({'username': 'zolgus', 'points': 600}));
...
describe('User: getBetStats', () => {
    test('Bet Stats', async () => {
        const [userId] = await dbConn('users').insert(f.user({'username': 'snorlax', 'points': 100}));
